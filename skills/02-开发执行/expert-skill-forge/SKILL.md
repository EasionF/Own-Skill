...
| **6. 真实验证** | 真的跑一次端到端案例，让局外人评判 | 局外人能看出"像专家做的" **+ 必须在 accumulated/index.md 追加一条锻造记录，否则本次不算完整** | 读 phase-guides.md §6 + references/self-check.md |

> **关于第六阶段的强制写入**：`accumulated/` 不是可选的"建议沉淀"，是流程里的强制最后一步。如果跳过这一步，本次锻造不算完整——这跟"生成了字段但没人消费"是同一个反模式：设计了积累机制却没有接入执行流程，用完就忘，积累层会一直空着。详见 [accumulated/index.md](accumulated/index.md) 顶部的强制声明。
