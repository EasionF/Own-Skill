...
用于说明通用设计原则、版式规范和视觉决策约束，供设计相关阶段参考。
...
- Text inside a container must never cross the container boundary.
- Border-crossing text is a layout failure, not a styling preference.
- Multiple text blocks inside the same container must not visually overlap each other; keep a clear gap between emphasis lines, annotations, and body copy.
- Do not solve overflow only by shrinking the font size.
- Preferred fix order:
  1. shorten the copy;
  2. add a manual line break;
  3. widen or heighten the container;
  4. change the layout;
  5. only then reduce the font size, and still keep every glyph fully inside.
