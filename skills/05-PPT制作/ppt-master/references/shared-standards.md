...
用于 `ppt-master` 各角色共享的技术规范，包括 SVG 约束、兼容性要求、画布规格和公共执行边界。

## 审查要点

- 是否被 Strategist、Executor、Image_Generator 等阶段正确引用。
- 是否作为共享约束而非局部风格建议使用。
- 是否与导出链路和 PPT 兼容要求保持一致。
...
- **Chinese wrapping rule**: Never rely on generic English-oriented auto-wrap logic for Chinese copy. Chinese text must be wrapped by an explicit CJK-safe line-break rule, and must not produce broken fragments such as `Ap p`, `国 内`, or `品 牌`
- **Narrow containers**: Pills, footer strips, KPI cards, contact cards, and other narrow containers must wrap long text with `<tspan>` instead of forcing single-line text
...
- **Coordinate rule**: Avoid leaving layout-critical content inside unflattened `transform="translate(...)"` groups in final `svg_output/`. Flatten or bake coordinates into child elements before audit/export so the preflight tool and PPT exporter read the same geometry
- **Shared helper rule**: Mixed CJK/English copy must use the shared CJK-safe wrapping helper, and nested `translate(...)` groups must be recursively flattened before audit/export

---

## 5. Pre-export Validation Gate

Before the 3-step post-processing pipeline, the following command is mandatory:
...
**Zero-tolerance rule**:
- No visible overlap
- No text overflow outside cards / strips / chart areas
- No axis-label collision with chart borders
- No contact-card / footer text crowding
- No CJK/English mixed-word corruption caused by bad line wrapping
- No audit/export geometry mismatch caused by unresolved transform groups

If the audit reports any issue, `svg_output/` must be fixed first. Proceeding to finalize/export is forbidden.
