...
用于 `ppt-master` 的图像生成阶段，负责根据设计规格中的图像资源清单生成提示词、调用图像生成工具并产出图片资源。

## 功能数据流转

Strategist 输出设计规格中的图像资源清单  
-> Image_Generator 生成标准化 prompt  
-> 调用图像工具生成图片  
-> 保存到 `images/` 目录  
-> 交由 Executor 使用
...
- 是否严格依据图像资源清单生成 prompt。
- 是否正确保存 prompt 文档和图片文件。
- 是否保证文件命名与项目路径一致。
