...
FONT_FILE_CANDIDATES = {
    ...
    'Microsoft JhengHei': [r'C:\Windows\Fonts\msjh.ttc', r'C:\Windows\Fonts\msjh.ttf'],
    'Microsoft JhengHei Bold': [r'C:\Windows\Fonts\msjhbd.ttc', r'C:\Windows\Fonts\msjhbd.ttf'],
    'SimSun': [r'C:\Windows\Fonts\simsun.ttc'],
    'SimSun Bold': [r'C:\Windows\Fonts\simsun.ttc'],
    'SimHei': [r'C:\Windows\Fonts\simhei.ttf'],
    'SimHei Bold': [r'C:\Windows\Fonts\simhei.ttf'],
    'Consolas': [r'C:\Windows\Fonts\consola.ttf'],
    'Consolas Bold': [r'C:\Windows\Fonts\consolab.ttf'],
}


def _font_key(family: str, weight: str) -> str:
    bold = weight in ('bold', '600', '700', '800', '900')
    family = (family or '').strip()
    if family in GENERIC_FONT_MAP:
        family = GENERIC_FONT_MAP[family]
    if family in FONT_FALLBACK_WIN:
        family = FONT_FALLBACK_WIN[family]
    if family:
        bold_key = f'{family} Bold'
        if bold and bold_key in FONT_FILE_CANDIDATES:
            return bold_key
        if family in FONT_FILE_CANDIDATES:
            return family
    return 'Segoe UI Bold' if bold else 'Segoe UI'
...
def measure_text_width(text: str, font_family: str, font_size: float, font_weight: str = '400') -> float:
    """Measure text width using actual font metrics when available; fall back to heuristic."""
    if ImageDraw is not None and Image is not None:
        ...
        if font is not None:
            ...
            try:
                ...
                try:
                    ...
                    pass
    return estimate_text_width(text, font_size, font_weight)
...
def _build_text_runs(elem: ET.Element, parent_attrs: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Backward-compatible flattened text runs for width estimation."""
    lines = _build_text_run_lines(elem, parent_attrs)
...
def _build_text_run_lines(elem: ET.Element, parent_attrs: Dict[str, Any]) -> List[List[Dict[str, Any]]]:
    """Build line-aware text runs from SVG <text>/<tspan>."""
    ...
    if elem.text:
        ...
        if t:
            current_line.append({**parent_attrs, 'text': t})
    ...
    for child in elem:
        ...
        if t:
            ...
            current_line.append({**run_attrs, 'text': t})
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> str:
    ...
    # Build line-aware runs from <text> + <tspan> children
    parent_attrs = {
        'fill': fill_color,
        'fill_raw': fill_raw,
    ...
    line_runs = [line for line in _build_text_run_lines(elem, parent_attrs) if line]
    runs = [run for line in line_runs for run in line]
    ...
    for line in line_runs:
        ...
        dominant_family = next((r.get('font_family') for r in line if r.get('font_family')), font_family_str)
        dominant_weight = next((r.get('font_weight') for r in line if r.get('font_weight')), font_weight)
        dominant_size = max(r.get('font_size', font_size) for r in line)
        line_widths.append(measure_text_width(line_text, dominant_family, dominant_size, dominant_weight) * 1.06)
        line_heights.append(dominant_size * 1.42)

    text_width = max(line_widths) if line_widths else measure_text_width(full_text, font_family_str, font_size, font_weight) * 1.06
    text_height = sum(line_heights) if line_heights else font_size * 1.5
    padding = max(font_size * 0.18, 4)
    container_width = _f(elem.get('data-width')) * ctx.scale_x
    if container_width > 0:
        text_width = max(text_width, max(container_width - padding * 2, 0))
    ...
    # Build paragraph XML (preserve explicit SVG line breaks from <tspan>)
    paragraphs_xml = []
    for line in line_runs:
        runs_xml = '\n'.join(_build_run_xml(r, fonts, ctx) for r in line)
        paragraphs_xml.append(f'<a:p>\n<a:pPr algn="{algn}"/>\n{runs_xml}\n</a:p>')
    paragraphs = '\n'.join(paragraphs_xml)
    wrap_mode = 'square' if len(line_runs) > 1 else 'none'
